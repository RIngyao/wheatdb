...
snp_table_server <- function(id, snps_df) {
  moduleServer(id, function(input, output, session){
    ...
    observe({
      ...
      ge_checks <- unlist(lapply(gene_list(), check_gene, "name"))
      if(!all(ge_checks)){
        idx <- which(ge_checks == FALSE)
        errorId <- gene_list()[idx]
    ...
    #     observe({
    #      # browser()
    #       gene <- unlist(strsplit(gene_sample(), "[, ]+")) %>% unique()
    #        req(input$upload)
    #
    #         output$table_output <- renderDT({
    #           file_content <- read_delim(input$upload$datapath, col_names = "gene", delim = ",")
    #           split_gene <- lapply(unlist(file_content), check_gene, "size")
    #           ind <- which(split_gene == FALSE)
    #              return(file_content)
    #
    # #           if(isTRUE(ind)) {
    # #             return(gene()[ind])
