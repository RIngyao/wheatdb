...
# sample_list <- reactive(colnames(snps_df))
    ...
    observe({
     # req(is.data.frame(snps_df), sample_list())
      ...
      llst <- dbConnect(con,
                       "SELECT GROUP_CONCAT(COLUMN_NAME ORDER BY ORDINAL_POSITION SEPARATOR CHAR(9)) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'wheatdb' AND TABLE_NAME = 'vcf_table' AND ORDINAL_POSITION >= 8")
     ...
     final_table <- eventReactive(input$click,{
      ...
      # check for error and extract the data
       ...
       if(query() == "geneID") {
         ...
         # query <- "select * from vcf_table where gene;"

         dbGetQuery(con, sqlInterpolate(con, "SELECT * FROM vcf_table WHERE GENE_ID = ?"))
      ...
      # }
         ...
         if(isTRUE(track_error_df$status) && isTruthy(start_coord$coord()) && isTruthy(end_coord$coord())){
           ...
           if(query() == "coordinates") {
             ...
             dbGetQuery(con, sqlInterpolate(con, "SELECT * FROM vcf_table WHERE CHROM = ? AND POS >= ? AND POS <= ?"))
             ...
             dbGetQuery(con, sqlInterpolate(con, "SELECT * FROM vcf_table WHERE CHROM = ? AND POS >= ? AND POS <= ? AND TYPE = ?"))
            ...
            # df_table <- df_sample()[df_sample()$POS >= start_coord$coord() & df_sample()$POS <= end_coord$coord() & df_sample()$TYPE == type_sample() & df_sample()$`#CHROM` == chr_sample(), col_list]
             ...
             dbGetQuery(con, sqlInterpolate(con, "SELECT * FROM vcf_table WHERE CHROM = ? AND POS >= ? AND POS <= ? AND IMPACT = ?"))
     ...
     df_plot <- reactive({
       ...
       if(query() == "geneID") {
         ...
         if(isTRUE(gene_error()) && (nrow(final_table()) > 1)) {
           ...
           dbGetQuery(con, sqlInterpolate(con, "SELECT * FROM vcf_snpeffect WHERE GENE_ID = ?"))
         ...
         if(isTRUE(track_error_df$status) && isTruthy(start_coord$coord()) && isTruthy(end_coord$coord())) {
           if(nrow(final_table()) > 1) {
             if(query() == "type") {
               ...
               dbGetQuery(con, sqlInterpolate(con, "SELECT * FROM vcf_snpeffect WHERE CHROM = ? AND POS >= ? AND POS <= ? AND TYPE = ?"))
              # df <- snps_df %>% filter(`#CHROM` == chr_sample() & (POS >= start_coord$coord() & POS <= end_coord$coord()) & TYPE == type_sample())
               ...
               dbGetQuery(con, sqlInterpolate(con, "SELECT * FROM vcf_snpeffect WHERE CHROM = ? AND POS >= ? AND POS <= ? AND IMPACT = ?"))
               ...
               dbGetQuery(con, sqlInterpolate(con, "SELECT CHROM, POS FROM vcf_snpeffect WHERE CHROM = ? AND POS >= ? AND POS <= ?"))
